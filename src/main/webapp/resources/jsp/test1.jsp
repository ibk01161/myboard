...
window.onload = function() {
	document.getElementById('question_count').onchange = function() {
		...
		for(let cnt = 1; cnt <= kv_count; cnt++) {
			kv_temp += '<tr>';
			kv_temp += '<td><input type="text" name="key" placeholder="key" required /></td>';
			kv_temp += '<td><input type="text" name="value" placeholder="value" required /></td>';
			kv_temp += '</tr>';
		}
		kv_temp += '</table>';
...
<tr>
	<td><div id="kv">
		...
		<tr>
			...
			placeholder="key" required="required" /></td>
			<td><input type="text" name="value" placeholder="value"  required="required" /></td>
		</tr>
		</table>
		</div>
	</td>
